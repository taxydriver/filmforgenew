...
import type { PortraitResult } from "@/lib/generatePortraits";
import type {
  PromptOptions,
  VisualStyle,
  Lens,
...
export type StoryboardCharactersPayload = {
  ...
  portraits?: Record<string, PortraitResult[]>;
};
...
export default function StoryboardCharactersStep({
  ...
  const [portraitState, setPortraitState] = useState<Record<
    string,
    {
      ...
      results: { pose: string; url?: string; error?: string }[];
      status?: string;
      error?: string;
    }
  >>({});
    ...
    if (data) {
      ...
      if (data.portraits) {
        setPortraitState((prev) => {
          const next = { ...prev };
          Object.entries(data.portraits as Record<string, PortraitResult[]>)
    ...
    try {
      ...
      while (true) {
        ...
        while ((idx = buffer.indexOf("\n\n")) !== -1) {
          ...
          setPortraitState((prev) => {
            const current = prev[c.name] || { loading: true, progress: {}, results: [] };
            ...
            if (msg.type === "status") {
              if (msg.state === "done" && Array.isArray(msg.results)) {
                return {
                  ...prev,
                  [c.name]: { ...current, loading: false, status: "done", results: msg.results },
                };
              }
      ...
      setPortraitState((prev) => {
        ...
        return {
          ...prev,
          [c.name]: { ...current, loading: false, status: current.status === "starting" ? "done" : current.status },
        };
      });
    } catch (err: any) {
      setPortraitState((prev) => ({
